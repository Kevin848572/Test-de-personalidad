...
const EMPTY_SCORES: ScoreVector = { E: 0, I: 0, S: 0, N: 0, T: 0, F: 0, J: 0, P: 0 };

function pickHigher(a: number, b: number, left: string, right: string): string {
  return a >= b ? left : right;
}

export function buildScoreVector(answers: AnswerMap): ScoreVector {
  const scores: ScoreVector = { ...EMPTY_SCORES };
  ...
  for (const q of questions) {
    ...
    if (letter in scores) scores[letter as keyof ScoreVector]++;
  }

  return scores;
}

export function resolveCode(scores: ScoreVector): MBTICode {
  ...
  return (e + s + t + j) as MBTICode;
}

export function computeResult(answers: AnswerMap): TestResult {
...
export function getDimensionPairs(scores: ScoreVector) {
